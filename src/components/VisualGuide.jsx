...
import CareCanvas from './CareCanvas.jsx'
      ...
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-4 p-4 text-left sm:p-5">
        ...
        <span className="flex-1">
          <span className="block text-[11px] font-bold uppercase tracking-wider text-slate-400">
            Langkah {n + 1} dari {count} • {step.step}
          </span>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="flex items-start gap-4">
          <span className="mt-1 hidden sm:block">
            <Icon name="sparkle" className="h-7 w-7 text-amber-500" />
          </span>
          <div>
            <h2 className="text-xl font-extrabold text-slate-900 sm:text-2xl">Sasaran Minggu Pertama</h2>
            <p className="mt-2 max-w-2xl text-base leading-relaxed text-slate-600">{protocol.goal}</p>
          </div>
        </div>
        <div className="mt-5 flex flex-wrap gap-2">
          <span className="rounded-full px-3.5 py-1.5 text-xs font-bold" style={{ backgroundColor: `${meta.accent}1a`, color: meta.accent }}>
            {protocol.visual.length} langkah panduan
          </span>
          <span className="rounded-full bg-slate-100 px-3.5 py-1.5 text-xs font-bold text-slate-600">
            Acuan: {meta.refSource.split('(')[0].trim()}
          </span>
        </div>
      </div>

      <CareCanvas accent={meta.accent} roomName={meta.short} />
